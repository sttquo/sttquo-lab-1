#include <iostream>
using namespace std;
int main() {
    // task 1

    setlocale(0, "Rus");
    cout << "int: " << sizeof(int) << " байт\n";
    cout << "short int: " << sizeof(short int) << " байт\n";
    cout << "long int: " << sizeof(long long) << " байт\n";
    cout << "float: " << sizeof(float) << " байт\n";
    cout << "double: " << sizeof(double) << " байт\n";
    cout << "long double: " << sizeof(long double) << " байт\n";
    cout << "char: " << sizeof(char) << " байт\n";
    cout << "bool: " << sizeof(bool) << " байт\n";

    cout << "\n" << "\n" << "\n";


    // task 2

    
    int integerNumber;
    unsigned int mask = 1 << (sizeof(int) * 8 - 1);

    cout << "¬ведите число типа integer: ";
    cin >> integerNumber;
    for (int i = 0; i < sizeof(int) * 8 + 1; ++i, mask >>= 1) { 
        if (mask & integerNumber)
            cout << '1';
        else
            cout << '0';

        if (i == 1 || i % 8 == 0) {

            cout << ' ';

        }
    }

    cout << "\n" << "\n" << "\n";

    
    // task 3

    
    union {

        int numberI;
        float numberF;

    };
    cout << "¬ведите число типа float: ";
    cin >> numberF;

    /* cout << &numberI << " " << &numberF << "\n"; -- проверка на то, что numberI и numberF занимают одно и то же место в пам€ти
    cout << numberI << " " << numberF << "\n"; -- просмотр того, как компьютер преобразовал число типа float в число типа integer
    */

    mask = 1 << (sizeof(float) * 8 - 1);

    for (int i = 0; i < sizeof(float) * 8 + 1; ++i, mask >>= 1) {
        if (mask & numberI)
            cout << '1';
        else
            cout << '0';

        if (i == 1 || i % 8 == 0) {

            cout << ' ';
        }
            

    }


}